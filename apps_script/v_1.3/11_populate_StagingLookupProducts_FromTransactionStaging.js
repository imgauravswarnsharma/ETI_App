 ...
 *
 * Purpose:
 * - Populate Staging_Lookup_Products with unresolved product entries
 * - One staging row per product canonical (forward-only intake)
 * - Acts as review + promotion intake for Product master data
 * - Mirrors Item and Brand staging behavior exactly (ETI v1.3 consistency)
 *
 * Preconditions:
 * - Sheets must exist:
 ...
 * 2. Load Staging_Lookup_Products and build an in-memory set of
 *    existing Product_Name_Canonical values.
 ...
 *    b. Skip if Product_ID_Machine already exists.
 *    c. Skip if Product_Name_Canonical is missing.
 ...
 * - Superseded only if product ingestion moves to ID-first
...
function populateStagingLookupProducts_FromTransactionResolution() {
  ...
  const IDX_STG_CANON = stgCol('Prodcut_Name_Canonical');
  if (IDX_STG_CANON === -1) {
    throw new Error('Staging_Lookup_Products missing Prodcut_Name_Canonical');
  }
  ...
  for (let i = 1; i < tsData.length; i++) {
    ...
    newRow[stgCol('Staging_Prodcut_ID_Machine')]  = Utilities.getUuid(); // full UUID
    newRow[stgCol('Mapped_Prodcut_ID_Machine')]   = '';
    newRow[stgCol('Prodcut_Name_Entered')]        = r[IDX.productEntered];
    newRow[stgCol('Prodcut_Name_Canonical')]      = canon;
    newRow[stgCol('Prodcut_Name_Approved')]       = '';
